...
class MissingTranslationAnnotator : Annotator {
    ...
    override fun annotate(element: PsiElement, holder: AnnotationHolder) {
        if (element !is KtArrayAccessExpression) return
        if (!translationManager.isMessageExpression(element))  return

        val translation = translationManager.findTranslation(element)

        if (translation == null) {
            holder.newAnnotation(HighlightSeverity.WEAK_WARNING, "Missing translation")
                .range(element.textRange)
                .withFix(MissingTranslationQuickfix(element))
        }
    }
}
